...
struct SharedTacticalMapView: View {
    @ObservedObject var model: SharedGameModel
    ...
    private var spriteManifest: SharedSpriteManifest {
        SharedSpriteManifest.shared(for: model.runtimeResources)
    }

    init(model: SharedGameModel, scrollable: Bool = true) {
        self.model = model
    ...
    private func mapBaseImage() -> NSImage? {
        if let basePath = model.mapLevels.first(where: { $0.isBase && !$0.imagePath.isEmpty })?.imagePath,
           let image = SharedImageCache.shared.image(atPath: basePath) {
            return image
        }

        return SharedImageCache.shared.image(atPath: model.mapOverviewPath)
    }

    @ViewBuilder
    private func mapLevelImage(_ level: SharedMapLevel, layout: MapLayout) -> some View {
        if let image = SharedImageCache.shared.image(atPath: level.imagePath) {
            Image(nsImage: image)
                .resizable()
                .interpolation(.high)
    ...
    private func objectiveMarker(_ objective: SharedObjective, at point: CGPoint, layout: MapLayout) -> some View {
    ...
    private func objectiveLabel(_ objective: SharedObjective, at point: CGPoint, layout: MapLayout) -> some View {
    ...
    private func civilianMarker(_ civilian: SharedCivilian, at point: CGPoint) -> some View {
    ...
    private func civilianRiskMarker(_ civilian: SharedCivilian, at point: CGPoint) -> some View {
    ...
    private func contactMarker(_ contact: SharedContact, at point: CGPoint) -> some View {
    ...
    private func interactionMarker(_ interaction: SharedInteraction, at point: CGPoint, layout: MapLayout) -> some View {
    ...
    private func routeDestinationMarker(_ unit: SharedUnit, at point: CGPoint) -> some View {
    ...
    private func trafficVehicleDestinationMarker(_ vehicle: SharedTrafficVehicle, at point: CGPoint) -> some View {
    ...
    private func trafficVehicleMarker(_ vehicle: SharedTrafficVehicle, at point: CGPoint, layout: MapLayout) -> some View {
    ...
    private func trafficVehicleLabel(_ vehicle: SharedTrafficVehicle, at point: CGPoint, layout: MapLayout) -> some View {
    ...
    private func unitMarker(_ unit: SharedUnit, at point: CGPoint, layout: MapLayout) -> some View {
    ...
    private func unitLabel(_ unit: SharedUnit, at point: CGPoint, layout: MapLayout) -> some View {
    ...
    private func unitMarkerSize(_ unit: SharedUnit, layout: MapLayout) -> CGSize {
    ...
    private func unitStatusCount(_ unit: SharedUnit) -> Int {
        var count = 0
    ...
    private func unitStatusStack(_ unit: SharedUnit) -> some View {
        VStack(spacing: 2) {
            if !unit.selectionMarkerID.isEmpty {
    ...
    private func casualtyMarker(_ unit: SharedUnit, markerID: String) -> some View {
    ...
    private func mapLevelOpacity(_ level: SharedMapLevel) -> Double {
    ...
    private func mapLevelControlColor(_ level: SharedMapLevel) -> Color {
        if level.id.contains("roof_access") {
    ...
    private func contactColor(_ contact: SharedContact) -> Color {
        switch contact.kind {
        case 0:
    ...
    private func contactSymbol(_ contact: SharedContact) -> String {
        switch contact.kind {
        case 0:
            return "flame.fill"
        case 1:
    ...
    private func interactionSymbol(_ interaction: SharedInteraction) -> String {
    ...
    @ViewBuilder
    private func unitGlyph(_ unit: SharedUnit, layout: MapLayout) -> some View {
        let size = unitSpriteSize(unit, layout: layout)

        if let sprite = spriteManifest.unitSprite(for: unit),
           let image = SharedImageCache.shared.image(atPath: sprite.path) {
            ZStack {
                Circle()
    ...
    @ViewBuilder
    private func trafficVehicleGlyph(_ vehicle: SharedTrafficVehicle, layout: MapLayout) -> some View {
        let size = trafficVehicleSpriteSize(vehicle, layout: layout)
        let color = trafficVehicleColor(vehicle)
    ...
    private func unitSpriteSize(_ unit: SharedUnit, layout: MapLayout) -> CGFloat {
        if unit.side == 3 {
    ...
    private func trafficVehicleSpriteSize(_ vehicle: SharedTrafficVehicle, layout: MapLayout) -> CGFloat {
        switch vehicle.kind {
        case 1:
    ...
    private func trafficVehicleMarkerSize(_ vehicle: SharedTrafficVehicle, layout: MapLayout) -> CGSize {
        let size = trafficVehicleSpriteSize(vehicle, layout: layout)
        return CGSize(width: size + 36, height: size + 30)
    }

    private func trafficVehicleShortName(_ vehicle: SharedTrafficVehicle) -> String {
        switch vehicle.kind {
        case 1:
            return "Bus"
        case 2:
    ...
    private func trafficVehicleSymbol(_ vehicle: SharedTrafficVehicle) -> String {
        switch vehicle.kind {
        case 1:
            return "bus.fill"
        case 2:
    ...
    private func trafficVehicleColor(_ vehicle: SharedTrafficVehicle) -> Color {
        switch vehicle.kind {
        case 1:
...
final class SharedImageCache {
    ...
    func stats() -> SharedImageCacheStats {
        SharedImageCacheStats(
